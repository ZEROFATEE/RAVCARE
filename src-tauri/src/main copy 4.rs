    ...
    #[tauri::command]
    fn open_home_window(app: tauri::AppHandle) {
        if let Some(window) = app.get_webview_window("queue") {
            let _ = window.show();
            let _ = window.set_focus();
        } else {
            println!("Queue window not found");
        }
    }
...
fn get_inventory_amount(state: tauri::State<AppState>, id: String) -> Result<f64, String> {
    ...
    struct Vaccine {
        ...
        dose_size: Option<f64>,
        last_edited: Option<String>,
    }

    #[tauri::command]
    fn get_vaccines(state: State<'_, AppState>) -> Result<Vec<Vaccine>, String> {
        let conn = state.db.lock().unwrap();

        let mut stmt = conn
            .prepare(
                "SELECT id, name, category, route_type, dose_size, last_edited 
                FROM vaccines ORDER BY name",
            )
            .map_err(|e| e.to_string())?;
        ...
        let rows = stmt
            .query_map([], |row| {
                Ok(Vaccine {
                    ...
                    route_type: row.get::<_, Option<String>>(3)?,
                    dose_size: row.get::<_, Option<f64>>(4)?,
                    last_edited: row.get::<_, Option<String>>(5)?,
                })
            })
            .map_err(|e| e.to_string())?;
    ...
    fn create_vaccine(
        ...
        let route_final = route_type.trim().to_string();
        let dose_final = if dose_size > 0.0 { dose_size } else { 1.0 };
        ...
        conn.execute(
            "INSERT INTO vaccines (id, name, category, route_type, dose_size, last_edited)
            VALUES (?1, ?2, ?3, ?4, ?5, datetime('now','localtime'))",
            params![id, name, category_clean, route_final, dose_final],
        )
        .map_err(|e| e.to_string())?;
    ...
    fn update_vaccine(
        ...
        conn.execute(
            ...
            SET name = ?1,
                ...
                route_type = ?3,
                dose_size = ?4,
                last_edited = datetime('now','localtime')
            WHERE id = ?5",
            params![name, category_clean, route_final, dose_final, id],
        )
        .map_err(|e| e.to_string())?;
    ...
    #[tauri::command]
    fn delete_vaccine(state: State<'_, AppState>, id: String) -> Result<String, String> {
        let conn = state.db.lock().unwrap();

        conn.execute("DELETE FROM vaccines WHERE id = ?", params![id])
            .map_err(|e| e.to_string())?;
    ...
    let encrypted_password_clone = encrypted_password.clone(); // ✅ add this
    let now_clone = chrono::Utc::now().naive_utc().to_string(); // ✅ add this
    tokio::spawn(async move {
        if let Err(e) = insert_user_to_supabase(
            &username_clone,
            &password_hash_clone,
            patient_id,
            &password_plain_clone,
            &secret_key_clone,
            &encrypted_password_clone,
            &now_clone,
        ).await {
...
pub async fn sync_all_musers_to_supabase() -> Result<String, String> {
    ...
    // 1️⃣ Select ALL columns we need
    let mut stmt = conn
        .prepare(
            "SELECT id, username, password_hash, patient_id, encrypted_password, created_at
             FROM musers"
        )
        .map_err(|e| e.to_string())?;
    ...
    let rows = stmt
        .query_map([], |row| {
            Ok((
                ...
                row.get::<_, i64>(3)?,    // patient_id
                row.get::<_, String>(4)?, // encrypted_password
    ...
    // 2️⃣ Push every row to Supabase
    for (id, username, password_hash, patient_id, encrypted_password, created_at) in rows {
        let body = json!({
            "id": id,
            "username": username,
...
async fn insert_appointment_to_supabase(
    patient_id: i64,
    schedule: &str,
    prophylaxis_type: Option<&str>,
    regular_type: Option<&str>,
) -> Result<(), String> {
    let client = Client::new();
    let url = "https://ddfzwwaldprstwgjbuvp.supabase.co/rest/v1/appointments";
    let key = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImRkZnp3d2FsZHByc3R3Z2pidXZwIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NjMyMzMwNzAsImV4cCI6MjA3ODgwOTA3MH0.My0LeEJ1LS96YYNA4YlmpoSwIGEyYWjq-tyCtt2IJKI";
    ...
    let body = json!({
        ...
        "day_zero_date": prophylaxis_type.map(|_| schedule), // quick default
...
/* ➋ mirror the local insert + background sync */
#[tauri::command]
fn create_appointment_with_sync(
    state: State<'_, AppState>,
    appointment: Appointment,          // existing struct you already use
) -> Result<String, String> {
    let conn = state.db.lock().unwrap();

    /* ---- LOCAL INSERT (unchanged) ---- */
    conn.execute(
        "INSERT INTO appointments (
            patient_id, schedule, prophylaxis_type, regular_type,
            day_zero_date, day_three_date, day_seven_date,
            day_fourteen_date, day_thirty_date, status
    ...
    let pid = appointment.patient_id as i64; 
    let sched = appointment.schedule.clone();
    let proph = appointment.prophylaxis_type.clone(); // owned Option<String>
        let reg   = appointment.regular_type.clone();  

    tokio::spawn(async move {
    // pass references to the cloned values – they live inside the spawned task
    if let Err(e) = insert_appointment_to_supabase(
        pid,
        &sched,
        proph.as_deref(),
        reg.as_deref(),
    ...
    .await
    {
        eprintln!("❌ Supabase appointment sync failed: {}", e);
    } else {
        println!("✅ Appointment synced to Supabase");
    }
});
...
fn update_muser_password(
   ...
   async fn update_user_in_supabase(
    ...
    let res = client
        .patch(format!("{}/rest/v1/muser?username=eq.{}", supabase_url, username))
        .header("apikey", supabase_key)
        .header("Authorization", format!("Bearer {}", supabase_key))
        .header("Content-Type", "application/json")
        .header("Prefer", "return=minimal")
...
async fn request_verification(
    ...
    .setup(|app| {
        // 1️⃣ Create the queue window if it does not exist
        if app.get_webview_window("queue").is_none() {
            tauri::WebviewWindowBuilder::new(
                app,
                "queue",
                tauri::WebviewUrl::App("queue".into()),
            )
            .title("Queue Display")
            .resizable(true)
            .decorations(true)
            .build()
            .expect("Failed to build queue window");
        }

        // 2️⃣ Now the window exists -> attach close handler
        let app_handle = app.handle().clone();
        if let Some(queue) = app.get_webview_window("queue") {
            queue.on_window_event(move |event| {
                if let tauri::WindowEvent::CloseRequested { api, .. } = event {
                    api.prevent_close();            // ❗ prevent destruction
                    if let Some(win) = app_handle.get_webview_window("queue") {
                        let _ = win.hide();         // ✔ hide instead of close
                    }
                }
            });
        }
