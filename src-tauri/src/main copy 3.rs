...
pub async fn sync_all_musers_to_supabase() -> Result<String, String> {
    // 1️⃣  ALL DB work first – NO await yet
    let rows = {
...
let rows = stmt
    ...
    .map_err(|e| e.to_string())? // ✅ add ?
    .collect::<Result<Vec<_>, _>>()
    .map_err(|e| e.to_string())?;
        rows
    }; // 🔥 Connection dropped here – no longer in future

    // 2️⃣  NOW we can use await freely
    let client = reqwest::Client::new();
    let supabase_url = "https://ddfzwwaldprstwgjbuvp.supabase.co";
    let supabase_key = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImRkZnp3d2FsZHByc3R3Z2pidXZwIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NjMyMzMwNzAsImV4cCI6MjA4ODgwOTA3MH0.My0LeEJ1LS96YYNA4YlmpoSwIGEyYWjq-tyCtt2IJKI";

    let mut ok = 0;
    let mut fail = 0;

    for (id, username, hash, pid, enc_pw, created) in rows {
        let body = json!({
            "id": id,
            "username": username,
            "password_hash": hash,
    ...
    Ok(format!("✅ synced {}  ❌ failed {}", ok, fail))
}
