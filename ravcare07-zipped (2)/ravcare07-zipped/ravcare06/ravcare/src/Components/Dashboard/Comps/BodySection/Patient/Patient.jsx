...
async function handleSaveRegularAppointment(e) {
  ...
  // -------------------------------------
  // 1️⃣ Duplicate Prevention (same type + date)
  // -------------------------------------
  const duplicateConflict = appointments.some((a) => {
    if (editRegular && a.id === editRegular.id) return false; // Skip itself
  ...
  if (duplicateConflict) {
    alert(
      `⚠️ This patient already has a ${newRegularType} scheduled on ${newRegularDate}.`
    );
    return;
  }
 ...
 let payload = {
  ...
  // 🔥 Use real date like anti-rabies does
  schedule: 
    newRegularType === "Hepa B Vaccine"
      ? newRegularDate        // Hepa B uses first-dose date
      : newRegularGivenDate,  // Others use “Date Given”
  ...
  if (newRegularType === "Hepa B Vaccine") {
    ...
    if (newRegularType === "Flu Vaccine") {
      payload.vaccine_name = newRegularName;
    }
  ...
  try {
    ...
    // -------------------------------------
    // 6️⃣ If editing → refresh that record only
    // -------------------------------------
    if (editRegular) {
    ...
    // -------------------------------------
    // 7️⃣ If creating → auto-open latest regular vaccine
    //     (same behavior as anti-rabies)
    // -------------------------------------
    const latestRegular = updatedList
      .filter((v) => v.regular_type) // Only regular records
...
const markRegularDone = async (appt) => {
  try {
    ...
    const updateField = async (id, field, value) => {
      await invoke("status_apointment_update", { id, field, value });
    };

    // Only Hepa B uses Mark Done
    const doses = [
    ...
    await updateField(appt.id, dose.given, localToday);
    await updateField(appt.id, dose.status, "✔️ Done");
...
{/* TETANUS TOXOID                */}
{/* ------------------------------ */}
{viewRegularData.regular_type === "Tetanus Toxoid" && (
  <>
    <p>
      <b>Date Given:</b>{" "}
{viewRegularData.regular_given_date && viewRegularData.regular_given_date.trim() !== ""
...
{viewRegularData.regular_type !== "Hepa B Vaccine" &&
 !["Flu Vaccine", "Pneumonia Vaccine", "Tetanus Toxoid"].includes(viewRegularData.regular_type) && (
  <>
    <p>
      <b>Date Given:</b>{" "}
{viewRegularData.regular_given_date && viewRegularData.regular_given_date.trim() !== ""
...
<div className="regbtns">
  ...
  <button
    ...
    onClick={() => {
      ...
      const doc = iframe.contentWindow.document;
      doc.open();
      doc.write(`<html><body>${printContents}</body></html>`);
      doc.close();
    ...
    <div className="card-print" style={{ width: "5.5in", height: "4in" }}>
      ...
      <table>
        ...
        <tbody>
          ...
          {viewRegularData.regular_type !== "Hepa B Vaccine" && (
            <tr>
              <td>{viewRegularData.regular_date}</td>
          ...
          {viewRegularData.regular_type === "Hepa B Vaccine" && (
            <>
              <tr>
                <td>{viewRegularData.hepa_b_dose1}</td>
                <td>{viewRegularData.hepa_b_given1 || "—"}</td>
                <td></td>
              </tr>
              <tr>
                <td>{viewRegularData.hepa_b_dose2}</td>
                <td>{viewRegularData.hepa_b_given2 || "—"}</td>
                <td></td>
              </tr>
              <tr>
                <td>{viewRegularData.hepa_b_dose3}</td>
